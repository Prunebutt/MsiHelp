...
\begin{tcolorbox}[colback=yellow!5!white,colframe=yellow!75!white,coltitle=black,title=\textbf{Bayesian Estimation and the Maximum a Posteriori Estimate}]
	...
	\hat{\theta}_{MAP} &= \underset{\theta\in \mathbb{R}}{\argmin} \{ -\log (p (y_N| \theta) ) -\log (p(\theta))\}
	\end{flalign*}
	
	\textbf{MAP Example:} Regularised Least Squares
	\begin{flalign*}
	\theta &= \bar \theta \pm \sigma_\theta \quad \text{with} \quad \bar \theta =  \theta_{\text{a-priori} } &\\
	\hat \theta_{MAP} &= \underset{\theta \in \mathbb{R}}{\argmin} \frac{1}{2} \cdot \frac{1}{\sigma_{\epsilon^2 }} \cdot \lVert y_N - \Phi_N \cdot \theta \rVert_{2}^{2} + \frac{1}{2} \cdot \frac{1}{\sigma_{\theta}^2} \cdot (\theta - \bar \theta)^2
	\end{flalign*}
\end{tcolorbox}
...
\begin{flalign*}
	& \hat \theta_{ML} = \underset{\theta \in \mathbb{R}^d}{arg\,max} \  p(y|\theta ) = \underset{\theta \in \mathbb{R}^d}{\argmin} \sum_{i=1}^{N} \frac{(y_i - M_i (\theta))^2}{2 \sigma_{i}^{2}} & \\
	& = \underset{\theta}{\argmin} \frac{1}{2} \sum_{i=1}^{N} \left(\frac{y_i - M_i (\theta)}{\sigma_i }\right)^2 & \\
	& = \underset{\theta \in \mathbb{R}^d}{\argmin} \frac{1}{2} \lVert S^{-1} (y - M(\theta) )\rVert_{2}^{2} \qquad
...
\begin{flalign*}
	\theta_{ML}(N) &= \underset{\theta \in \mathbb{R}}{\argmin} \frac{1}{2} \lVert y_N - \Phi_N \cdot \theta \rVert_{2}^{2} \qquad \text{(forgetting factor: } \alpha )&\\
	\hat \theta_{ML} (N+1) &= \underset{\theta \in \mathbb{R}^d}{\argmin} \left(\alpha \cdot \frac{1}{2} \cdot \lVert \theta - \hat \theta_{ML}(N) \rVert Q_N^2 \right. & \\
