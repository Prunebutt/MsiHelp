...
\begin{flalign*}
	...
	& \textbf{Innovation update: } \hat \theta_k = \hat \theta_{k-1} + \underset{"`innovation"'}{\underbrace{Q_{k}^{-1} \phi_k (y_k - \phi_{k}^{T} \hat \theta_{k-1})} } &\\
	& \textbf{General Optimization Problem: } & \\
	& \hat \theta_k = \underset {\theta}{arg\,min} (\theta -\hat \theta_{0})^{T} \cdot Q_0 \cdot (\theta -\hat \theta_0) + \sum_{i=1}^{k} (y_k - \phi_k^T \cdot \theta)^2 &
\end{flalign*}
\end{tcolorbox}
